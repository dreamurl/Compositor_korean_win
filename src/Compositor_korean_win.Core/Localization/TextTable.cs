...
public static class TextTable
{
    ...
    [
        ...
        (TextKey.AiRulesDefault,
            ...
            + "10. Write each element pixel for pixel: work out its pixels yourself, save a PNG with transparency in exact colours and put_pixels it onto its own layer. No magic-wand cut-outs, no reduced colours.\n"
            + "10a. Work out coordinates and outlines yourself from the reference; never ask the person to click or select.\n"
            + "11. Make shapes and effects with masks, add_path, liquify, warp_layer and distort_layer. Do not paint them by hand.\n"
            + "12. Keep lettering as text layers. The background is the scene with elements and lettering taken out and their places filled in.\n"
            ...
            + "\n"
            + "## 레퍼런스 재현\n"
            + "10. 요소는 픽셀 단위로 옮긴다: 요소의 픽셀을 직접 알아내 정확한 색의 투명 배경 PNG로 만들고 put_pixels로 자기 레이어에 넣는다. 마술봉 누끼 금지, 색 줄이기 금지.\n"
            + "10a. 좌표와 윤곽은 레퍼런스에서 직접 알아낸다. 사용자에게 클릭이나 선택을 부탁하지 않는다.\n"
            + "11. 모양과 효과는 마스크, add_path, liquify, warp_layer, distort_layer로 만든다. 손으로 따라 칠하지 않는다.\n"
            + "12. 글자는 텍스트 레이어로 둔다. 배경은 요소와 글자를 걷어내고 그 자리를 채운 장면이다.\n"
        ...
        (TextKey.AiWorkflow,
            "## How to work\n"
            + "\n"
            + "- Work everything out yourself from what you are given. Look at the picture with `render` and read files with a script for exact colours and positions; decide coordinates, outlines and sizes yourself. Never ask the person to click, select or give coordinates.\n"
            + "- Plan the layers before drawing: one element per layer — each person or object, the background scene, shapes (masks, cut-outs), textures and accents, text — named by role and grouped.\n"
            + "- A reference's elements are written pixel for pixel. Work out which pixels belong to each element, save it as a PNG with transparency in the reference's exact colours, and `put_pixels` it onto its own layer. Never reduce the colours, and do not cut elements out with the magic wand — its edges are rough.\n"
            + "- The background is the scene with the elements and lettering taken out and the places they covered filled in, not a second copy of the reference.\n"
            + "- Shapes and effects are made with the editor's tools, not traced by hand: silhouettes and cut-outs with `add_path` (as mask or selection) and `set_mask`; bends and perspective with `liquify`, `warp_layer` and `distort_layer`; tone with `add_adjustment`; shadows and outlines with `set_effects`. `paint_stroke` is for touching up and filling in, not for carrying pixels.\n"
            + "- Text is always a text layer (`add_text`), never pixels.\n"
            ...
            "## 작업 방법\n"
            + "\n"
            + "- 모든 것을 주어진 자료에서 스스로 알아낸다. 그림은 `render`로 보고, 파일은 스크립트로 읽어 정확한 색과 위치를 얻는다. 좌표, 윤곽, 크기는 직접 정한다. 사용자에게 클릭이나 선택, 좌표를 부탁하지 않는다.\n"
            + "- 그리기 전에 레이어를 계획한다. 요소 하나에 레이어 하나: 인물이나 물체 하나하나, 배경 장면, 모양(마스크, 잘라낸 부분), 질감과 포인트, 텍스트. 역할이 드러나게 이름을 붙이고 그룹으로 묶는다.\n"
            + "- 레퍼런스의 요소는 픽셀 단위로 옮긴다. 요소마다 어느 픽셀이 그 요소인지 알아내 레퍼런스의 정확한 색으로 투명 배경 PNG를 만들고, `put_pixels`로 자기 레이어에 넣는다. 색 수를 줄이지 않는다. 마술봉으로 누끼를 따지 않는다 — 가장자리가 거칠다.\n"
            + "- 배경은 요소와 글자를 걷어내고 그 자리를 채운 장면이다. 레퍼런스를 한 벌 더 복사한 것이 아니다.\n"
            + "- 모양과 효과는 편집기 기능으로 만들고 손으로 따라 그리지 않는다. 실루엣과 잘라낸 틈은 `add_path`(마스크나 선택 영역으로)와 `set_mask`, 휘어짐과 원근은 `liquify`·`warp_layer`·`distort_layer`, 색조는 `add_adjustment`, 그림자와 윤곽선은 `set_effects`로 만든다. `paint_stroke`는 다듬기와 채우기에 쓰고, 픽셀을 옮기는 데 쓰지 않는다.\n"
            + "- 텍스트는 항상 텍스트 레이어(`add_text`)로 만들고 픽셀로 두지 않는다.\n"
        ...
        (TextKey.AiTopicReproduce,
            ...
            + "\n"
            + "The aim is the reference's look, built the way its maker built it: each element on its own layer in its exact pixels, the background whole behind them, shapes and effects made with masks and transforms, and live text. Work it all out yourself from the reference — never ask the person for coordinates or to select anything.\n"
            + "\n"
            + "1. Look and plan. Read the reference file with a script for its size, exact colours and positions, and look at it (`render` once it is on the canvas). List the elements — each person and object, the background scene, shapes that cut things, textures, text — and plan a layer or group for each, named by role.\n"
            + "2. Canvas. `new_document` at the reference's size, so its pixels and the canvas's share coordinates. `put_pixels` the whole reference onto a layer named Reference, hide it with `update_layer` visible false, and delete it at the end.\n"
            + "3. Elements, pixel for pixel. Do not cut elements out with the magic wand or a rough selection. For each element, decide pixel by pixel which pixels are its own: trace its outline from the reference, then settle the edge by the colours on either side — hair, fingers and thin or see-through edges take partial alpha. Write the element as a PNG with transparency, the reference's exact colours inside and nothing outside, and `put_pixels` it at its position onto a new layer named after the element. Paint in, with `paint_stroke` in the element's own colours, any part that something in front of it hid. `render` the layer alone, zoomed along its edges, and correct the PNG until the edge is exact.\n"
            + "4. Background. The background is the scene with every element and all the lettering taken out and the places they covered filled in. `put_pixels` the reference onto a Background layer; for each element, `select` shape layer on the element's layer, `modify_selection` expand 2, and `fill_selection` content_aware true on Background; do the same over the lettering (`select` rectangles or `add_path` as=selection round it). Touch up with `paint_stroke` tool heal or clone. Then hide the element layers and `render`: nothing of them may be left in the background.\n"
            + "5. Shapes. Cut-outs and silhouettes are masks, never painted: `add_path` as=mask (fill_rule evenodd for holes, mode subtract for gaps), or `select` then `set_mask` shape selection. Feather where the reference is soft.\n"
            + "6. Effects. Bends, swirls and perspective come from `liquify`, `warp_layer` and `distort_layer`; tone from `add_adjustment`; shadows and glows from `set_effects`. Do not trace an effect pixel by pixel.\n"
            + "7. Text. `add_text` for every piece of lettering, matching font (`list_fonts`), size, tracking, leading, colour and position; `edit_text` with start and end for letters styled apart. Never keep the reference's lettering as pixels.\n"
            + "8. Check. `compare_image` against the reference; read the score and the worst regions, `render` them with zoom, fix, and compare again. Delete the Reference layer. Report the final score and what still differs.",
            "# 레퍼런스 재현\n"
            + "\n"
            + "목표는 레퍼런스를 만든 사람이 만든 방식 그대로 같은 모습을 만드는 것이다. 요소마다 정확한 픽셀로 자기 레이어에, 그 뒤에는 온전한 배경, 모양과 효과는 마스크와 변형으로, 텍스트는 살아 있는 텍스트로 만든다. 모든 것을 레퍼런스에서 스스로 알아낸다 — 사용자에게 좌표나 선택을 부탁하지 않는다.\n"
            + "\n"
            + "1. 보고 계획한다. 레퍼런스 파일을 스크립트로 읽어 크기, 정확한 색, 위치를 얻고, 캔버스에 올린 뒤 `render`로 본다. 요소를 나눈다 — 인물과 물체 하나하나, 배경 장면, 무언가를 잘라내는 모양, 질감, 텍스트 — 요소마다 레이어나 그룹을 계획하고 역할로 이름을 붙인다.\n"
            + "2. 캔버스. `new_document`를 레퍼런스 크기로 만들어 좌표를 같게 한다. `put_pixels`로 레퍼런스 전체를 Reference 레이어에 넣고, `update_layer`로 visible false로 숨기고, 마지막에 지운다.\n"
            + "3. 요소를 픽셀 단위로. 마술봉이나 대강의 선택으로 요소를 따지 않는다. 요소마다 어느 픽셀이 그 요소인지 픽셀 단위로 정한다: 레퍼런스에서 윤곽을 따라가고, 가장자리는 양쪽 색을 보고 정한다 — 머리카락, 손가락, 얇거나 비치는 가장자리는 반투명 알파를 준다. 요소를 투명 배경 PNG로 만들어(안은 레퍼런스의 정확한 색, 밖은 비움) `put_pixels`로 제자리에, 요소 이름을 붙인 새 레이어에 넣는다. 앞에 있는 것이 가리던 부분은 `paint_stroke`로 그 요소의 색으로 그려 넣는다. 그 레이어만 `render`로 가장자리를 따라 확대해 보고, 가장자리가 정확해질 때까지 PNG를 고친다.\n"
            + "4. 배경. 배경은 모든 요소와 글자를 걷어내고 그 자리를 채운 장면이다. `put_pixels`로 레퍼런스를 Background 레이어에 넣고, 요소마다 그 요소 레이어에서 `select` shape layer, `modify_selection` expand 2, Background에 `fill_selection` content_aware true. 글자 위도 똑같이 한다(`select` 사각형이나 `add_path` as=selection으로 둘러싼다). 다듬기는 `paint_stroke` tool heal이나 clone. 그다음 요소 레이어를 숨기고 `render`로 본다: 배경에 요소가 조금도 남아 있으면 안 된다.\n"
            + "5. 모양. 잘라낸 부분과 실루엣은 마스크로 만들고 칠하지 않는다. `add_path` as=mask(구멍은 fill_rule evenodd, 틈은 mode subtract), 또는 `select` 후 `set_mask` shape selection. 레퍼런스가 부드러운 곳은 feather를 준다.\n"
            + "6. 효과. 휘어짐, 소용돌이, 원근은 `liquify`·`warp_layer`·`distort_layer`, 색조는 `add_adjustment`, 그림자와 빛은 `set_effects`로 만든다. 효과를 픽셀 단위로 따라 그리지 않는다.\n"
            + "7. 텍스트. 모든 글자를 `add_text`로 만들고 글꼴(`list_fonts`), 크기, 자간, 행간, 색, 위치를 맞춘다. 일부 글자만 다른 스타일은 `edit_text`의 start와 end로. 레퍼런스의 글자를 픽셀로 남기지 않는다.\n"
            + "8. 확인. `compare_image`로 레퍼런스와 비교해 점수와 가장 다른 영역을 읽고, 그 영역을 `render` zoom으로 보고 고친 뒤 다시 비교한다. Reference 레이어를 지운다. 마지막 점수와 아직 다른 부분을 보고한다."),
