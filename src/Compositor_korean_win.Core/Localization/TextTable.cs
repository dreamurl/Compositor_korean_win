...
public static class TextTable
{
    ...
    [
        ...
        (TextKey.AiRulesDefault,
            ...
            + "14. Never separate an element with the magic wand, even when asked for a cut-out: judge its boundary pixel by pixel and put it on its own layer.\n"
            + "15. Fill where lettering or elements were so it continues the background without a seam: gradients from the edge colours, textures matched in brightness and colour to the edge, lines carried through, never a blur. Check the border zoomed in.",
            "# Compositor 작업 규칙\n"
            + "\n"
            ...
            + "14. 요소를 분리할 때 마술봉을 쓰지 않는다. 누끼 요청도 같다: 경계를 픽셀 단위로 판단해 따로 레이어에 담는다.\n"
            + "15. 글자나 요소가 있던 자리는 배경과 이음새 없이 채운다: 그라데이션은 경계 색에서 잇고, 무늬는 밝기와 색을 경계에 맞추고, 선은 이어 그리고, 흐림으로 메우지 않는다. 경계를 확대해서 확인한다."),
        ...
        (TextKey.AiWorkflow,
            ...
            + "- A reference's material comes across exactly with `put_pixels`: every colour as it is, never reduced to fewer colours, each element on its own layer.\n"
            + "- Never separate an element with the magic wand — for a reference, and also when asked for a cut-out. Judge the element's boundary pixel by pixel, exactly, and put the element on a layer of its own; hair and soft edges stay partly transparent.\n"
            ...
            + "- Text is always a text layer (`add_text`), never pixels. Where lettering or an element covered the picture, fill it so it continues the background without a seam: gradients continued from the colours at the edge, textures filled from around and matched in brightness and colour to the edge, lines carried through unbroken — never a blur. Look at the border zoomed in.\n"
            ...
            + "- 레퍼런스의 재료는 `put_pixels`로 정확히 옮긴다. 색은 원본 그대로 두고 색 수를 줄이지 않는다. 요소마다 자기 레이어에 넣는다.\n"
            + "- 요소를 분리할 때 마술봉을 쓰지 않는다 — 레퍼런스 재현이든 누끼 요청이든 같다. 요소의 경계를 픽셀 단위로 정확히 판단해 따로 레이어에 담는다. 머리카락과 부드러운 가장자리는 반투명으로 둔다.\n"
            ...
            + "- 텍스트는 항상 텍스트 레이어(`add_text`)로 만들고 픽셀로 두지 않는다. 글자나 요소가 가리던 자리는 배경과 이음새 없이 이어지게 채운다: 그라데이션은 경계 색에서 이어 가고, 무늬는 주변에서 채운 뒤 밝기와 색을 경계에 맞추고, 선은 끊기지 않게 잇는다 — 흐림으로 메우지 않는다. 경계를 확대해서 본다.\n"
        ...
        (TextKey.AiTopicReproduce,
            ...
            + "4. Material. For each element, judge its boundary pixel by pixel — never with the magic wand — select exactly those pixels (`select` lasso or `add_path` as=selection along the edge, checked zoomed in until every edge pixel is right), then `put_pixels` with within_selection true onto the element's own layer. Colours stay exactly as they are. For the base material take everything behind the shapes, not only what shows.\n"
            + "5. What was hidden. Where text or other elements covered the material, fill it in so it continues the background without a seam — a colour step or a smudge shows at once.\n"
            + "   - Take only what covered it: the lettering's own pixels with their soft edges, shadows and glows, not a wide rectangle.\n"
            + "   - Smooth colour (sky, a still water surface): continue the gradient from the colours at the edge; do not copy patches into it.\n"
            + "   - Texture (ripples, buildings, leaves): fill from the texture around it — `select` the gap and `fill_selection` content_aware true, or `paint_stroke` tool clone or heal — then match the brightness and colour to the edge, as Poisson blending does, so no border shows.\n"
            + "   - Lines that cross the gap (a horizon, ripple lines, the edge of a building): carry them through unbroken and straight.\n"
            + "   - Never fill with a blur: a smear shows more than the text did.\n"
            + "   - Check every filled area with `render` zoomed in on its border: no colour step, smudge or broken line.\n"
            ...
            + "4. 재료. 요소마다 경계를 픽셀 단위로 판단한다 — 마술봉은 쓰지 않는다. 정확히 그 픽셀들만 선택하고(가장자리를 따라 `select` lasso나 `add_path` as=selection, 가장자리 픽셀이 모두 맞을 때까지 확대해 확인), `put_pixels`에 within_selection true를 주어 그 요소의 레이어에 넣는다. 색은 원본 그대로다. 바탕 재료는 보이는 부분만이 아니라 모양 뒤에 있는 것까지 가져온다.\n"
            + "5. 가려진 부분. 텍스트나 다른 요소가 재료를 가리던 곳은 배경과 이음새 없이 이어지게 채운다 — 색이 조금만 달라도, 조금만 얼룩져도 바로 티가 난다.\n"
            + "   - 가린 것만 잡는다: 글자 픽셀과 그 부드러운 가장자리, 그림자, 빛번짐까지. 넓은 사각형으로 잡지 않는다.\n"
            + "   - 색이 서서히 변하는 곳(하늘, 잔잔한 수면): 경계의 색에서 그라데이션을 이어 간다. 패치를 복사해 넣지 않는다.\n"
            + "   - 무늬가 있는 곳(물결, 건물, 나뭇잎): 주변 무늬로 채운다 — 빈 곳을 `select`하고 `fill_selection` content_aware true, 또는 `paint_stroke` tool clone이나 heal — 그다음 밝기와 색을 경계에 맞춘다(푸아송 블렌딩처럼). 경계가 보이면 안 된다.\n"
            + "   - 빈 곳을 지나는 선(수평선, 물결 선, 건물 모서리): 끊기거나 비뚤어지지 않게 이어 그린다.\n"
            + "   - 흐림으로 메우지 않는다. 번진 얼룩은 글자보다 더 티가 난다.\n"
            + "   - 채운 곳마다 경계를 `render` zoom으로 확대해 본다: 색 차이, 얼룩, 끊긴 선이 없어야 한다.\n"
