...
| 변환 | 핸들 8개 + 회전 핸들, 이동·크기·회전·디스토트, Shift/Alt/비율 잠금 |
| 스냅 | 캔버스와 다른 레이어의 가장자리·중심, 축별 독립, 화면상 10pt 고정 |
| 선택 | 사각형·타원·올가미, 더하기/빼기, 스캔라인 커버리지(안티에일리어싱 4행 샘플) |
| 셸 | 휠 줌, 스페이스·가운데 버튼 팬, 드래그 변환, Ctrl+0/1, Ctrl+Z/Y, Ctrl+A/D |
...
선택 영역 안의 픽셀을 옮기고 변환하는 것과 다각형 올가미는 M4(도구), 조정 레이어 적용은 M5다.
디스토트는 계산(`TransformDrag.Corners`)만 있고 아직 셸에 연결하지 않았다.
