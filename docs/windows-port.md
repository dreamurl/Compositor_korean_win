...
| **M3** 캔버스 | 뷰포트·줌·팬·변환 핸들·스냅·가이드·선택 영역 | 프레임당 읽는 픽셀이 문서가 아니라 창에 비례(10.4) + WARP 프레임 시간 기록 |
...
### 10.4 M3의 완료 기준을 바꾼 이유

원래 기준은 "1억 픽셀 문서에서 60fps 유지"였다. **CI에서 증명할 수 없는 기준이다** — 러너에는
GPU가 없어 M0 보고서가 `driver: warp`로 남았고(10.1), 소프트웨어 래스터라이저에서 60fps는
나오지 않는다. 로컬 빌드를 하지 않기로 한 이상(2절 전제) 다른 측정 장소도 없다.
...
- **하한 기록**: WARP에서 잰 프레임 시간은 보고서에 남기되 판정에는 쓰지 않는다.
