...
#### 스트로크 지연 — 실측 (WARP, 붓 지름 40, 한 번에 3픽셀 이동)

| 항목 | 값 |
|---|---|
| 다음 점까지 잇는 시간 (100만 픽셀 레이어) | **0.21 ms** |
| 다음 점까지 잇는 시간 (**1억** 픽셀 레이어) | **0.21 ms** |
| 칠하는 중 한 프레임 | 4.8 ms |
| 스트로크가 만진 타일 | 2개 |
...
스트로크가 끝날 때 한 번뿐이다. 포인터 이동당 총지연은 0.21 + 4.8 ≈ 5ms — **GPU 없는 기계에서**다.
...
#### 실측 (GitHub Actions, GPU 없음 = WARP, 1280×800 창, 10,000² = 1억 픽셀 문서)
...
맞춤에서는 피라미드가 남긴 마지막 2배 이하를 `HighQualityCubic`으로 리샘플한다. 소프트웨어
래스터라이저에서 큐빅 커널은 픽셀당 수십 나노초를 쓰므로 50만 출력 픽셀이 곧 40밀리초다.
GPU에서는 이 단계가 텍스처 유닛 몫이라 사실상 공짜다. **즉 42ms는 "GPU 없는 기계에서도
이 정도"라는 하한이지 목표가 아니다.**
