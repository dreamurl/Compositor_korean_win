...
- **(M7에서 바뀜 — progress.md 8절)** 처음 계획은 모델을 번들하지 않고 내려받는 것이었다. 실제로는
  모델을 exe와 함께 싣는 **AI 포함판**과 exe만 있는 **미포함판**을 나눠 배포한다. 모델도 RMBG-1.4(비상업용)
  대신 BiRefNet-lite(MIT)로 정했다.
...
| 코드 서명 | SmartScreen 경고. **미서명으로 배포 중**(릴리스 노트에 안내). 유료 서명은 별도 판단 | M8 — 미해결 |
...
| ~~**M6** 주요 UI~~ ✅ | 레이어 패널·탭·시트 + 한국어 리소스 + 단축키 | **명시된 완료 기준 충족.** 세부 조작 패리티는 progress.md 11절 |
| ~~**M7** AI~~ ✅ | 피사체 분리(ONNX+DirectML) — 온디맨드 대신 포함판/미포함판으로 나눔 | **완료.** 미포함판도 자체검사 전부 통과. progress.md 8절 |
| ~~**M8** 배포~~ ✅ | 설치 파일, 업데이트 피드, Releases 자동화 | **완료.** v1.0.1을 받아 설치·실행 검증. progress.md 9절 |

M0–M8 모두 끝났다. 원본과 대조한 마무리 검토에서 찾은 자르기·선택 픽셀 변형·레이어 패널 조작·
스포이드·손·돋보기 등도 이후 보완했다. 구현 내역과 의도적으로 다르게 둔 동작은 progress.md 10절에 있다.
