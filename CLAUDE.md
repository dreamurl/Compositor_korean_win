...
- 셸 자체검사는 한 `CanvasView` 에서 앞 검사의 상태를 이어 쓴다. 마스크 선택처럼 다음 검사에 영향을
  주는 상태는 명시적으로 되돌리고, 상태를 유지하는 동작 자체가 요구사항이면 유지 전·후를 각각 확인한다.
...
그 뒤로 포스터 재현을 목표로 텍스트 레이어·뒤틀기·도형 확장·레이어 효과를 넣었다(`docs/progress.md` 13절).
PSD 가져오기·내보내기도 넣었다(14절). 이어서 `--mcp` MCP 서버(Codex·OpenAI·명령 이미지 생성)를 넣고
CI에서 실제 exe로 포스터를 재현한다(15절, 사용법은 `docs/mcp.md`). 도구는 AI 손 도구·선택 영역·묶음 실행을 더해
44개다(28절). 남은 것은 실제 입력 확인, 실제 AI
클라이언트·Codex 이미지 생성으로 써 보기, 그리고 향후 과제로 미룬 Photoshop 실제 테스트다.

CI 결과물에 **내용을 확인하지 않은 외부 이미지를 쓰지 않는다.** 포스터 검사는 그림을 스크립트로 그린다(15.4).
