...
M0~M8 완료. 릴리스는 `v*` 태그 푸시로 만들어진다(`docs/progress.md` 9절). 최신 릴리스는 v1.0.1이다.
그 뒤 원본 대조에서 찾은 기능도 보완했다: 자르기, 선택 픽셀 변형, 조정 편집 메뉴, 픽셀 격자,
변형 컨트롤 표시, 레이어 패널 드래그·우클릭·마스크 썸네일 조작, 스포이드·손·돋보기 도구.
현재 상세와 의도적으로 다르게 둔 동작은 `docs/progress.md` 10절에 있다. C 커널은 전부 연결됐다.
